          ...
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="d-flex justify-content-between align-items-center">
                <div>
                  <h4 class="font-weight-bold mb-0 text-uppercase">Service categtory List</h4>
                </div>
                <div>
                    <button type="button" class="btn btn-primary btn-icon-text btn-rounded" data-toggle="modal" data-target="#category_create">
                        <i class="ti-plus"></i>Add New
                    </button>
                </div>
                @push('all_models')
                <form class="forms-sample" action="{{route('super.agent.categories.store')}}" method="post" enctype="multipart/form-data">
                 @csrf
                    <div class="modal fade" id="category_create" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        ...
                        @foreach ($categories as $row )                            
                            <tr>
                                ...
                                <td>
                                    ...
                                    @push('all_models')
                                    <form class="forms-sample" action="{{route('super.agent.categories.update',$row->id)}}" method="post" enctype="multipart/form-data">
                                    @method('put')
                                    @csrf
                                    ...
                                    @push('all_models')
                                    <form action="{{route('super.agent.categories.destroy',$row->id)}}" method="post">
                                        @method('delete')
                                        @csrf
