...
<head>
    ...
    <title>Rodcem | {{ auth('company')->user()->name }}</title>
    <!-- plugins:css -->
...
<body>
    <div class="container-scroller">
        ...
        <a class="navbar-brand brand-logo-mini" href="{{route('front')}}"><img src="{{ asset('Dashboard/images/logo-mini.svg') }}" alt="logo"/></a> --}}
                <a class="navbar-brand brand-logo me-5" style="max-width: 80px;height: 41px;"
                    href="{{ route('company.dashboard') }}"><img src="{{ asset('assets/rodcem/logo.png') }}"
                ...
                <ul class="navbar-nav navbar-nav-right">
                    <li class="nav-item dropdown" id="notification_list">
                        <a class="nav-link count-indicator dropdown-toggle" id="notificationDropdown" href="#"
                            ...
                            <span class="count" id="count">{{ company_notification_count('company', auth('company')->id()) }}</span>
                        </a>
                        {{-- style="height: 300px;overflow:scroll " --}}
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown"
                            aria-labelledby="notificationDropdown">
                            <p class="mb-0 font-weight-normal float-left dropdown-header">Notifications</p>
                            @foreach (company_notification(current_guard(), auth('company')->id()) as $notification)
                                @if ($loop->index <= 4)
                                ...
                                @endif
                            @endforeach
                            <a href="{{ route('company.all.notification') }}" class="dropdown-item">View All</a>
                        </div>
                    </li>
                        ...
                        <a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown"
                            ...
                            @if (Auth()->guard('company')->user()->logo)
                                <img class="card-img-top"
                                    src="{{ asset('company/profile/logo/' . auth('company')->user()->logo) }}"
                                    alt="company logo">
                            @else
                                <img class="card-img-top" src="{{ asset('assets/rodcem/defautllogo.webp') }}"
                                    alt="default logo">
                            @endif
                        </a>
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown"
    ...
    <script>
        ...
        var channel = pusher.subscribe('company_notification_channel_{{ auth('company')->id() }}');
        channel.bind('company_notification_event_{{ auth('company')->id() }}', function(data) {
    ...
    <script>
        $('#notificationDropdown').click(function() {
            ...
            $.ajax({
                type: 'get',
                url: "{{ route('company.notification.read') }}",
