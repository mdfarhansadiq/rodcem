...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="row">
                    ...
                    <div class="form-group">
                        <div class="card" style="width: 8rem;">
                            @if (Auth()->guard('expert')->user()->image)
                                <img class="card-img-top" src="{{asset('expert/profile/'.Auth()->guard('expert')->user()->image)}}" alt="Card image cap">
                            @else
                                <img class="card-img-top" src="{{asset('assets/rodcem/profile.png')}}" alt="Card image cap">
                            @endif
                        </div>
                    </div>
                    <form class="forms-sample" action="{{route('expert.profile.update')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="exampleInputUsername1">User Name</label>
                            <input type="text" class="form-control" id="exampleInputUsername1" name="name" value="{{Auth()->guard('expert')->user()->name}}">
                        ...
                        <div class="form-group">
                            ...
                            <select name="designation" class="form-control" id="exampleFormControlSelect2">
                                @foreach (expert_categories() as $category)
                                  <option value="{{$category->id}}" {{Auth('expert')->user()->designation == $category->id ? 'selected' : ''}}>{{$category->name}}</option>
                                @endforeach
                            </select>
                            @error('designation')
                        ...
                        <div class="form-group">
                            <label for="exampleFormControlSelect2">Service Zone</label>
                            <select name="service_zone" class="form-control" id="exampleFormControlSelect2">
                                @foreach (all_district() as $item)
                                    <option value="{{$item->id}}" {{$item->id == auth('expert')->user()->service_zone ? 'selected' : '' }}>{{$item->name}}</option>
                                @endforeach
                            </select>
                            @error('service_zone')
                                <span class="text-danger">{{$message}}</span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="exampleInputUsername1" name="phone_number" value="{{Auth()->guard('expert')->user()->phone_number}}">
                            @error('phone_number')<div class="text-danger">{{$message}}</div> @enderror
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Email address</label>
                            <input type="email" class="form-control" id="exampleInputEmail1" name="email" value="{{Auth()->guard('expert')->user()->email}}">
            ...
            {{-- <div class="col-md-4 mx-auto grid-margin">
                <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Your Location</h4>
                    <form class="forms-sample" action="{{route('expert.location.update')}}" method="post">
                        @csrf
                        @if ($location)
                            <div class="form-group">
            ...
            <div class="col-md-4 mx-auto grid-margin">
                ...
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{route('expert.password.update')}}" method="post">
                    @csrf
                        <div class="form-group">
