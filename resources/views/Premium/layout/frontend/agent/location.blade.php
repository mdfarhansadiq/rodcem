...
@section('title')
    {{config('app.name')}} |  {{auth('agent')->user()->name}} | Location
@endsection
@section('location')
    active
@endsection
                    ...
                    <div class="form-floating theme-form-floating">
                        <select class="form-select" name="division" id="divsision" aria-label="Floating label select example">
                            ...
                            @foreach (division() as $item )
                                @if (agent_location(auth('agent')->id()))
                                    <option value="{{$item->id}}" {{($item->id == agent_location(auth('agent')->id())->division_id) ? 'selected' : ''}}>{{$item->name}}</option>
                                @else
                        ...
                        <select name="district" class="form-select" id="district" aria-label="Floating label select example">
                            <option selected="">Choose Your District</option>
                                @foreach (district() as $item )
                                    @if (agent_location(auth('agent')->id()))
                                        <option value="{{$item->id}}" {{($item->id == agent_location(auth('agent')->id())->district_id) ? 'selected' : ''}}>{{$item->name}}</option>
                                    @else
                                        <option value="{{$item->id}}">{{$item->name}}</option>
                                     @endif
                                @endforeach
                    ...
                    <div class="form-floating theme-form-floating">
                        <select name="upazila" class="form-select" id="upazila" aria-label="Floating label select example">
                            ...
                            @foreach (upazila() as $item ).
                                @if (agent_location(auth('agent')->id()))
                                    <option value="{{$item->id}}" {{($item->id == agent_location(auth('agent')->id())->upazila_id) ? 'selected' : ''}}>{{$item->name}}</option>
                                @else
                                    <option value="{{$item->id}}">{{$item->name}}</option>
                                @endif
                            @endforeach
                    ...
                    <div class="form-floating theme-form-floating">
                        <select class="form-select" name="union" id="union" aria-label="Floating label select example">
                            ...
                            @foreach (union() as $item )
                                @if (agent_location(auth('agent')->id()))
                                    <option value="{{$item->id}}" {{($item->id == agent_location(auth('agent')->id())->union_id) ? 'selected' : ''}}>{{$item->name}}</option>
                                @else
                                    <option value="{{$item->id}}">{{$item->name}}</option>
                                @endif
                            @endforeach
