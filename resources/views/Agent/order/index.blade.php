              ...
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="{{route('agent.user.orders')}}" class="btn btn-primary btn-icon-text btn-rounded">
                      <i class="ti-clipboard btn-icon-prepend"></i>User Order
                    </a>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-header">
                  ...
                  <form action="{{route('agent.order.search')}}" method="get">
                      <div class="input-group">
                        ...
                        <div class="input-group-append">
                          @if (request()->routeIs('agent.order.search'))
                            <button type="submit" class="btn btn-primary" type="button">Search</button>
                            <a href="{{route('agent.order.index')}}" type="submit" class="btn btn-danger" type="button">Cancel</a>
                          @else
                            <button type="submit" class="btn btn-primary" type="button">Search</button>
                          @endif
                        </div>
                  ...
                  <form action="{{route('agent.order.date.filter')}}" method="get">
                  <div class="row">
                      <div class="col-md-5">
                      ...
                      <div class="col-md-2">
                        <div class="form-group">
                          @if (request()->routeIs('agent.order.date.filter'))
                          <div class="d-flex justify-content-between">
                            <button class="btn btn-primary form-control mr-2" style="margin-top:30px">Filter</button>
                            <a href="{{route('agent.order.index')}}" type="submit" class="btn btn-danger" type="button" style="margin-top:30px">Cancel</a>
                          </div>
                          @else
                        ...
                        <tr>
                            ...
                            <td>
                              @if ($item->status == 'generated')
                                <span class="badge badge-pill bg-info text-capitalize">{{$item->status}}</span>
                              @elseif ($item->status == 'pending')
                                <span class="badge badge-pill bg-danger text-capitalize">{{$item->status}}</span>
                              ...
                              @elseif ($item->status == 'approve_by_admin')
                                <span class="badge badge-pill bg-danger text-capitalize">Processing</span>
                            ...
                            <td>
                                <a href="{{route('agent.order.show',$item->id)}}"><span class="badge badge-pill bg-primary">Order Details</span></a>
