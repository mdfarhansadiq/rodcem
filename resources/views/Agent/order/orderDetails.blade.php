        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                  <div>
                    <a href="{{route('agent.order.index')}}" type="button" class="btn btn-primary btn-icon-text btn-rounded">
          ...
          <div class="row">      
            ...
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <p class="card-title mb-0">Client Information</p>
                    <form action="{{route('agent.client.store',$order->id)}}" method="post">
                      @csrf
                      <div class="row">
                          <div class="form-group col-md-6">
                            <label for="exampleInputName1">Name <span class="text-danger">*</span></label>
                              <input type="text" name="name" class="form-control" id="exampleInputName1" placeholder="Enter Client Name" value="{{$order->client->name ?? ""}}" required>
                          </div>
                          <div class="form-group col-md-6">
                            <label for="exampleInputName1">Phone Number <span class="text-danger">*</span></label>
                              <input type="text" class="form-control" name="phone_number" id="exampleInputName1" placeholder="Client Phone Number" value="{{$order->client->phone_number ?? ""}}" required>
                          </div>
                          <div class="form-group">
                          ...
                          @csrf
                            ...
                            @endforeach
                             ...
                             <tr>
                              ...
                              <td></td>
                                <td>
                                  <a class="btn btn-primary" href="{{route('agent.order.index')}}">Back</a>
                                  @if ($order->status == 'generated' || $order->status == 'pending')                                    
                                  ...
                                  @if($order->status == 'payment_done' || $order->status == 'company_payment_receive' ||  $order->status == 'deliver' || $order->status == 'product_receive')
                                    ...
                                    @if($order->status == 'pending' || $order->status == 'approve_by_admin' || $order->status == 'approve_by_company')
                                      <a href="{{route('agent.order.cancel',$order->id)}}" class="btn btn-danger">Cancel</a>
                                    @endif
                                    {{-- cancel Order  --}}
