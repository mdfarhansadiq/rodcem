        ...
        <div class="row">
            <div class="col-md-12 mx-auto grid-margin stretch-card">
                ...
                <div class="card-body">             
                    ...
                    <form class="forms-sample" action="{{route('agent.document.update',$agent->slug)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        {{-- Shop Information --}}
                        <div class="row">
...
<script>
  $(function(){
      // All Divisions
      $('#parmanet_divsision').change(function(){
          var division_id = $(this).val();
          $.ajaxSetup({
              headers: {
                  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          ...
          $.ajax({
              ...
              data   : {'division_id' : division_id},
              success:function(response){
                $('#parmanet_district').html(response.html);
                $('#parmanet_upazila').html(response.district_upazilas);
                $('#parmanet_union').html(response.upazilas_union);  
              },
          });
      });

      // All upazilas
      $('#parmanet_district').change(function(){
          var district_id = $(this).val();
          $.ajaxSetup({
              headers: {
                  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          ...
          $.ajax({
              ...
              data   : {'district_id' : district_id},
              success:function(response){
                $('#parmanet_upazila').html(response.html);
              },
          });
      });

        // All unions
          $('#parmanet_upazila').change(function(){
          var upazila_id = $(this).val();
          $.ajaxSetup({
              headers: {
                  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          ...
          $.ajax({
              ...
              data   : {'upazila_id' : upazila_id},
              success:function(response){
                $('#parmanet_union').html(response.html);
              },
          });
      });
  });
