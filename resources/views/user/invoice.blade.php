...
<!-- Mirrored from angfuzsoft.com/html/invce/demo/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 20 Feb 2023 04:35:57 GMT -->

<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>www.rodcem.com</title>
    ...
    <link rel="apple-touch-icon" sizes="57x57" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-57x57.png">
    <link rel="apple-touch-icon" sizes="60x60" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-60x60.png">
    <link rel="apple-touch-icon" sizes="72x72" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-72x72.png">
    <link rel="apple-touch-icon" sizes="76x76" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-76x76.png">
    <link rel="apple-touch-icon" sizes="114x114" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-114x114.png">
    <link rel="apple-touch-icon" sizes="120x120" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-120x120.png">
    <link rel="apple-touch-icon" sizes="144x144" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-144x144.png">
    <link rel="apple-touch-icon" sizes="152x152" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-152x152.png">
    <link rel="apple-touch-icon" sizes="180x180" href="{{asset('invoice/assets')}}/img/favicons/apple-icon-180x180.png">
    <link rel="icon" type="image/png" sizes="192x192" href="{{asset('invoice/assets')}}/img/favicons/android-icon-192x192.png">
    <link rel="icon" type="image/png" sizes="32x32" href="{{asset('invoice/assets')}}/img/favicons/favicon-32x32.png">
    <link rel="icon" type="image/png" sizes="96x96" href="{{asset('invoice/assets')}}/img/favicons/favicon-96x96.png">
    <link rel="icon" type="image/png" sizes="16x16" href="{{asset('invoice/assets')}}/img/favicons/favicon-16x16.png">
    <link rel="manifest" href="{{asset('invoice/assets')}}/img/favicons/manifest.json">
    <meta name="msapplication-TileColor" content="#ffffff">
    <meta name="msapplication-TileImage" content="{{asset('invoice/assets')}}/img/favicons/ms-icon-144x144.png">
            ...
            <main>
                <div class="as-invoice invoice_style1">
                    <div class="download-inner" id="download_section">
                        <header class="as-header header-layout1">
                            <div class="row align-items-center justify-content-between">
                                <div class="col-auto">
                                    <div class="header-logo"><a href="{{route('front')}}"><img width="80" src="{{asset('assets/rodcem/logo.png')}}" class="me-2" alt="logo"/></a></div>
                                </div>
                                <div class="col-auto">
                                    <h1 class="big-title">Invoice 
                                        @if ($order->status == 'payment_done' || $order->status == 'company_payment_receive' || $order->status == 'product_receive' || $order->status == 'deliver')  
                            ...
                            <div class="header-bottom">
                                <div class="row align-items-center">
                                    <div class="col">
                                        <div class="border-line"><img src="{{asset('invoice/assets')}}/img/bg/line_pattern.svg" alt="line">
                                        </div>
                                    </div>
                                    <div class="col-auto">
                                        <p class="invoice-number me-4"><b>Invoice No: </b>#{{$order->same_order_uid}}</p>
                                    </div>
                                    <div class="col-auto">
                                        <p class="invoice-date"><b>Date: </b>{{$order->created_at->format('d-M-Y')}}</p>
                                    </div>
                                </div>
                            </div>
                        </header>
                        <div class="row justify-content-between mb-4">
                            <div class="col-auto">
                                <div class="invoice-left"><b>Invoiced To:</b>
                                    <address>{{$order->agent->name}}<br>{{$order->order_location->union->name}}, {{$order->order_location->upazila->name}}, {{$order->order_location->district->name}}<br>
                                     {{$order->order_location->divission->name}}
                                    </address>
                                </div>
                            </div>
                            <div class="col-auto">
                                <div class="invoice-right"><b>Pay To:</b>
                                    @if($order->user)
                                        <address>{{$order->user->name}}<br>{{$order->user->location->union->name}}, {{$order->user->location->upazila->name}}, {{$order->user->location->district->name}}<br>
                                            {{$order->user->location->divission->name}}
                                        </address>
                                    @else
                                        @if($order->client)
                                            <address>{{$order->client->name}}<br>{{$order->order_location->union->name}}, {{$order->order_location->upazila->name}}, {{$order->order_location->district->name}}<br>
                                                {{$order->order_location->divission->name}}
                                            </address>
                                        @endif
                                    @endif
                            ...
                            <tbody>                      
                                @foreach ($order->order_details as $detail )                                    
                                    <tr>                          
                                        ...
                                        <td>{{agent_total_price($order->id)/$detail->quantity}} <sup>Tk</sup> X {{$detail->quantity}} {{($detail->product->unit->name)}}</td>
                                        <td>{{$detail->agent_price}} <sup>Tk</sup></td>
                                    </tr>                          
                                @endforeach
                                <tr>
                                    <td colspan="5">&nbsp;</td>
                        ...
                        <div class="row justify-content-between">
                            ...
                            <div class="col-auto">
                                <table class="total-table">
                                    <tr>
                                        <th>Sub Total:</th>
                                        <td> {{agent_total_price($order->id)}} <sup>Tk</sup></td>
                                    </tr>
                                    <tr>
                                        <th>Tax:</th>
                                        <td>00.00 <sup>Tk</sup></td>
                                    </tr>
                                    <tr>
                                        <th>Total:</th>
                                        <td> {{agent_total_price($order->id)}} <sup>Tk</sup></td>
                    ...
                    <div class="text-center mt-2">
                        <p>Power By <a href="{{route('front')}}">www.rodcem.com</a></p>
                    </div>
                </div>
            </main>
        </div>
...
<!-- Mirrored from angfuzsoft.com/html/invce/demo/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 20 Feb 2023 04:36:03 GMT -->
